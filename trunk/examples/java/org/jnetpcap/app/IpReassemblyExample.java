...
public class IpReassemblyExample implements PcapPacketHandler<Object> {
	...
	public static class IpReassemblyBuffer
		 ...
		 * @param hash
		 *          special Ip4 based hash used for identifying this buffer quickly
		 */
		 ...
		 * @param offset
		 *          offset into this buffer where the fragment data should be copied
		 *          to
		 * @param length
	...
	/**
	 * Default buffer size to allocate for reassembly. Needs to be large enough to
	 * hold the Ip4 header upfront and the contetents of all fragment for a single
	 * fragmented Ip4 datagram.
	 */
	 ...
	 * "tests/test-ipreassembly2.pcap" and reassembles the IP fragments found into
	 * a new ip-only super packet. The new packet contains the Ip4 header as DLT.
	 * 
	 * @param args
	 *          ignored
	 */
	 ...
	 * Ip4 header is used to determine if a buffer already exists and if not a new
	 * one is created.
	 * 
	 * @param ip
	...
	private IpReassemblyBuffer getBuffer(Ip4 ip) {
		...
		if (buffer == null) { // First time we're seeing this id
			...
			buffer =
			    new IpReassemblyBuffer(ip, DEFAULT_REASSEMBLY_SIZE, bufTimeout, ip
			        .hashCode());
	 ...
	 * timed out buffer are dispatched, incomplete to the user's handler. Buffers
	 * that are still on the queue, are incomplete but have not timedout yet are
	 * ignored.
	 */
