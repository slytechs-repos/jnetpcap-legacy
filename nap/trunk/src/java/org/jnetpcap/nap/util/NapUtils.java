...
public class NapUtils {
	...
	/**
	 * Writes a file header to output stream. The header is written in the
	 * following format:
	 * 
	 * <pre>
	 ...
	 * @throws IOException
	 */
	public static int writePcapFileHeader(
	    ...
	    int accuracy,
	    int snaplen,
	    int dlt) throws IOException {

		final int PCAP_MAGIC_NUMBER = 0xa1b2c3d4;

		out.writeInt(PCAP_MAGIC_NUMBER);
	...
	/**
	 * Writes a file header to output stream with default values for version 2.0
	 * of pcap file header. The header is written in the following format:
	 * 
	 * <pre>
	 * typedef struct pcap_hdr_s {
	 * 	      guint32 magic_number;   // magic number 
	 ...
	 * @throws IOException
	 */
	public static int writePcapFileHeader(DataOutputStream out, int dlt)
	    throws IOException {

		return writePcapFileHeader(out, 2, 0, 0, 0, 64 * 1024, dlt);
	}

	/**
	 * Write a pcap packet header to output stream. The header is written in the
	 * following format:
	 * 
	 * <pre>
	 ...
	 * @throws IOException
	 */
	public static int writePcapPacketHeader(
	    DataOutputStream out,
	    int seconds,
	    int usecondes,
	    int included,
		...
		out.writeInt(usecondes);
		out.writeInt(included);
		out.writeInt(original);

		return 16;
	}

	/**
	 * Write a pcap packet header to output stream using defaults for current
	 * timestamp and original length. The header is written in the following
	 * format:
	 * 
	 * <pre>
	 * typedef struct pcaprec_hdr_s {
	 ...
	 * </pre>
	 * 
	 * Current time is used as a timestamp and included length as also the
	 * original length.
	 * 
	 * @param out
	 * @param included
	 ...
	 * @throws IOException
	 */
	public static int writePcapPacketHeader(DataOutputStream out, int included)
	    throws IOException {
		long ts = System.currentTimeMillis();
		long ns = System.nanoTime();

		final int seconds = (int) ts / 1000;
		final int usec = (int) ns / 1000;

		return writePcapPacketHeader(out, seconds, usec, included, included);
	}

	/**
	 * Creates a new file and write the supplied raw packet data into it.
	 * 
	 * @param path
	 * @param packets
	 *          raw packet data in hexdump format
	 * @return number of bytes written
	 * @throws IOException
	 */
	public static int createPcapFile(String path, String... packets)
	    throws IOException {
		byte[][] a = new byte[packets.length][];

		for (int i = 0; i < a.length; i++) {
			a[i] = NapUtils.toByteArray(packets[i]);
		}

		return createPcapFile(path, a);
	}
	 ...
	 * @throws IOException
	 */
	public static int createPcapFile(String path, byte[]... packets)
	    throws IOException {
		final File file = new File(path);
		DataOutputStream out = new DataOutputStream(new FileOutputStream(file));

		int count = NapUtils.writePcapFileHeader(out, PcapDLT.CONST_EN10MB);

		for (byte[] data : packets) {
			count += NapUtils.writePcapPacketHeader(out, data.length);
			out.write(data);
			count += data.length;
		}
