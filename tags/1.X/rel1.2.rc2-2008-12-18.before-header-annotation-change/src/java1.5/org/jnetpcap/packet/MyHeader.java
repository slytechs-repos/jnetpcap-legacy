...
public class MyHeader
	...
	public static abstract class Routing
		...
		private byte[] address(int index) {
	...
	public static int bindEthernet(JPacket packet, Ethernet eth, Ip4 ip) {
		return (eth.type() == 0x800) ? ip.hlen() * 4 : 0;
	}

	@Bind(to = IEEESnap.class)
	public static int bindSnap(JPacket packet, IEEESnap snap, Ip4 ip) {
		return ip.hlen() * 4;
	}
	...
	@Field(parent = "tos", offset = 2, length = 6, format = "%x", display = "code point")
	public int tos_Codepoint() {
		return (tos() & DIFF_CODEPOINT) >> 2;
	}

	@Field(parent = "tos", offset = 1, length = 1, format = "%x", display = "ECN bit")
	public int tos_ECN() {
		return (tos() & DIFF_ECT) >> 1;
	}

	@Field(parent = "tos", offset = 0, length = 1, format = "%x", display = "ECE bit")
	public int tos_ECE() {
	...
	public static int getHeaderLength(JBuffer buffer, int offset) {
		return (buffer.getUByte(0) & 0x0F) * 4;
	}
