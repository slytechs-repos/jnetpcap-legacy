...
import org.jnetpcap.packet.analysis.JAnalyze;
 ...
 */
public class Pcap implements JAnalyze {
	...
	public int analyze() {
		...
		return analyze(Pcap.LOOP_INFINATE);
	}
	...
	public int analyze(int count) {
		return analyze(count, null, null);
	}
	...
	public <T> int analyze(int count, JPacketHandler<T> handler, T user) {
		...
		final JController controller = JRegistry.getAnalyzer(JController.class);
		if (handler != null) {
			controller.add(handler, user);
		}

		return loop(count, controller, this);
	}
	...
	public <T> int analyze(JPacketHandler<T> handler, T user) {
		...
		return loop(Pcap.LOOP_INFINATE, controller, this);
	}
