...
public class Pcap {
	 ...
	 *         </p>
	 *         <p>
	 *         Look at the Filtering expression syntax section for details on the
	 *         str parameter.
	 *         </p>
	 * @deprecated use
	 ...
	 */
	@Deprecated
	public static int compile(int snaplen,
			int dlt,
	 ...
	 */
	public static boolean isLoaded(String methodName, Class<?>... parameterTypes) {
	 ...
	 * non-blocking mode and no packets were available to be read) or if no more
	 * packets are available in a ``savefile.'' A return of -1 indicates an error
	 * in which case or {@link #getErr()}() may be used to display the error text.
	...
	public <T> int loop(int cnt,
			final int id,
			final JPacketHandler<T> handler,
			T user) {
	...
	public <T> int loop(int cnt,
			final int id,
			final PcapPacketHandler<T> handler,
			T user) {
		return loop(cnt, new JBufferHandler<T>() {
	 ...
	 *         device, {@link Direction#OUT} will only capture packets sent by the
	 *         device and {@link Direction#INOUT} will capture packets received by
	 *         or sent by the device. {@link Direction#INOUT} is the default
	 *         setting if this function is not called.
	 *         <p>
	 ...
	 *         some other platforms might not support {@link Direction#OUT}.
	 *         </p>
	 *         <p>
	 *         This operation is not supported if a "savefile" is being read.
	 *         </p> {@link #ERROR} is returned, {@link #getErr()} gets the error
	 *         text.
	 * @since 1.4
	 * @see #isPcap100Supported()
	 * @author Sly Technologies, Inc.
	 ...
	 *         doesn't provide the proper typesafety. This is why we use a public
	 *         method first requiring a java enum type, and convert that to
	 *         appropriate native integer value.
	 *         </p> {@link #ERROR} is returned, {@link #getErr()} gets the error
	 *         text.
	 * @since 1.4
	 * @see #isPcap100Supported()
	 ...
	 * @param program
	 *          the program
	 * @return -1 is returned on failure, in which case {@link #getErr()}() may be
