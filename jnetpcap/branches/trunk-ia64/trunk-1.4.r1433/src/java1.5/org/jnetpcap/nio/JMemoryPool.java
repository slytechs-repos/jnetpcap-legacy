...
public class JMemoryPool {
	...
	public static class Block extends JMemory {
		...
		public synchronized int allocate(int size) {
			...
			assert JMemory.align(this.current) == 0 : "Alignment assertion error in JMemoryPool. Miss-alignment="
					+ JMemory.align(this.current);
...
//			int oldSize = size;
			...
			size += JMemory.align(size);
//			System.out.printf("JMemoryPool.allocate(%d) newSize=%d, "
//					+ "available=%d/%d, current=%d/%d%n",
//					oldSize,
//					size,
//					available,
//					available % 8,
...
//					current % 8);
	 ...
	 * on that boundary.
	 */
	private final static int BUS_WIDTH = JNumber.Type.LONG.size;
	 ...
	 * @param dst2
	 *            peered with same memory at src1.length offset using src2
	 *            length as length of peer
	 * @return total number of bytes duplicated
	 */
	public synchronized int duplicate2(JMemory src1, JMemory src2,
			JMemory dst1, JMemory dst2) {
	 ...
	 * @return total number of bytes duplicated
	 */
	public synchronized int duplicate2(JMemory src1, ByteBuffer src2,
			JMemory dst1, JMemory dst2) {
	...
	private Block newBlock(final int atLeastInSize) {
		return new Block((atLeastInSize > this.blockSize)
				? atLeastInSize
				: this.blockSize);
	}
