...
(JNIEnv *env, jclass clazz,	jstring jdevice, jint jsnaplen, jint jpromisc, 
		jint jtimeout, jobject jerrbuf) {
	 ...
	 */
	jobject pcap_obj = env->NewObject(clazz, pcapConstructorMID);
	setPhysical(env, pcap_obj, toLong(p));
...
//	}
//	
//	if (jpeer_ref_direct(env, node, (char *)p, 0, NULL)) {
//		return NULL;
//	}
//	
//	if (jmem_connect(env, pcap_obj, &node->h)) {
//		return NULL;
//	}
//	
//	/* 
//	 * Disable peering to our node which will prevent other Pcap classes from
//	 * peering not that there are any public methods for this. Lets be 
//	 * explicit though.
//	 */
//	jmem_allow_peering(&node->h, JMEM_FALSE);
...
JNIEXPORT jobject JNICALL Java_org_jnetpcap_Pcap_next__Lorg_jnetpcap_PcapHeader_2Lorg_jnetpcap_nio_JBuffer_2
  (JNIEnv *env, jobject obj, jobject header, jobject buffer) {
	...
	peer_t *pkt_peer = jpeer_get(env, buffer);
	if (pkt_peer == NULL) {
		return NULL;
	}

	jmemory_t *hdr_node = jmem_get(env, header);
	if (hdr_node == NULL) {
		return NULL;
	}
	
	pcap_pkthdr  *pkt_header = (pcap_pkthdr *)jmem_data_wo(hdr_node);
	if (pkt_header == NULL) {
		jnp_exception_code(env, PCAP_NEED_ALLOCATED_MEMORY);
		return NULL;
	}
	
	if (jmem_bounds(hdr_node, 0, sizeof(pcap_pkthdr))) {
		jnp_exception(env);
		return NULL;
	}
	
	char *pkt_data = (char *)pcap_next(p, pkt_header);
	if (pkt_data == NULL) {
		/*
		 * We don't throw exception here, we rely on libpcap provided error
		 * message in pcap.getErr() and NULL return value.
		 */
		return NULL; 
	}
	
	/*
	 * TODO: insert a proxy node between the peer and the direct data (pkt_data)
	 */
	if (jpeer_ref_direct(env, pkt_peer, pkt_data, (jint) pkt_header->caplen, obj)) {
		return NULL;
	}
...
JNIEXPORT jint JNICALL Java_org_jnetpcap_Pcap_nextEx__Lorg_jnetpcap_PcapHeader_2Lorg_jnetpcap_nio_JBuffer_2
  (JNIEnv *env, jobject obj, jobject header, jobject buffer) {
	...
	if (r != 1 || pkt_header == NULL || pkt_data == NULL) {
		return -1;
	}
		
	peer_t *hdr_peer = jpeer_get(env, header);
	if (hdr_peer == NULL) {
		return -1;
	}
	...
	// TODO: insert proxy node in between peer and direct data
	if (jpeer_ref_direct(env, hdr_peer, (char *)pkt_header, 
			sizeof(pcap_pkthdr), obj)) {
		return -1;
	}
	
	// TODO: insert proxy node in between peer and direct data
	if (jpeer_ref_direct(env, pkt_peer, (char *)pkt_data, 
			pkt_header->caplen, obj)) {
		return -1;
	}
...
JNIEXPORT jint JNICALL Java_org_jnetpcap_Pcap_lookupNet__Ljava_lang_String_2Lorg_jnetpcap_nio_JNumber_2Lorg_jnetpcap_nio_JNumber_2Ljava_lang_StringBuilder_2
  (JNIEnv *env, jclass clzz, jstring jdevice, jobject jnetp, jobject jmaskp, jobject jerrbuf) {
	...
	bpf_u_int32 *netp  = (bpf_u_int32 *) jmem_data_wo_get(env, jnetp);
	bpf_u_int32 *maskp = (bpf_u_int32 *) jmem_data_wo_get(env, jmaskp);
	if (netp == NULL || maskp == NULL) {
		return 0;
	}
